...
async function initMediaPipe() {
    ...
    try {
        ...
        for (let i = 0; i < TRIAL_LIMIT; i++) {
            trials.push({
                images: selectedIds.slice(i * 4, i * 4 + 4)
            });
        }
...
function drawDecision(trial, selectionIndex = -1) {
    ...
    // 计算2x2网格单个格子的最大宽高
    const maxCellW = (availableWidth - spacing) / 2;
    const maxCellH = (availableHeight - spacing) / 2;
    // 取较小值作为正方形边长
    const size = Math.min(maxCellW, maxCellH);

    // 居中偏移
    const offsetX = margin + (availableWidth - (size * 2 + spacing)) / 2;
    const offsetY = topMargin + (availableHeight - (size * 2 + spacing)) / 2;

    const coords = [
        { x: offsetX, y: offsetY }, // 左上
        { x: offsetX + size + spacing, y: offsetY }, // 右上
        { x: offsetX, y: offsetY + size + spacing }, // 左下
        { x: offsetX + size + spacing, y: offsetY + size + spacing } // 右下
    ];
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (img) {
            ...
            ctx.strokeRect(coords[i].x, coords[i].y, size, size);
            drawText(`1.jpg`, coords[i].x + size / 2, coords[i].y + size / 2, 20);
        }
    }
}
...
canvas.addEventListener('touchstart', (e) => {
    ...
    // 左上: 0, 右上: 1, 左下: 2, 右下: 3
    if (touchX >= offsetX && touchX <= offsetX + size && touchY >= offsetY && touchY <= offsetY + size) {
        tappedIndex = 0;
    } else if (touchX >= offsetX + size + spacing && touchX <= offsetX + size * 2 + spacing && touchY >= offsetY && touchY <= offsetY + size) {
        tappedIndex = 1;
    } else if (touchX >= offsetX && touchX <= offsetX + size && touchY >= offsetY + size + spacing && touchY <= offsetY + size * 2 + spacing) {
        tappedIndex = 2;
    } else if (touchX >= offsetX + size + spacing && touchX <= offsetX + size * 2 + spacing && touchY >= offsetY + size + spacing && touchY <= offsetY + size * 2 + spacing) {
        tappedIndex = 3;
    }
...
function handleDecision(selectionIndex) {
    ...
    setTimeout(() => {
        ...
        behaviorLog.push({
            ...
            top_left_img: trial.images[0],
            top_right_img: trial.images[1],
            bottom_left_img: trial.images[2],
            bottom_right_img: trial.images[3],
            chosen_position: selectionIndex, // 0:左上, 1:右上, 2:左下, 3:右下
